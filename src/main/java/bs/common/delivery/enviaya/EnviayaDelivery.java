...
import bs.common.delivery.enviaya.EnviayaRate.EnviayaRateResponse;
...
public abstract class EnviayaDelivery {
	...
	protected List<EnviayaRate> rates(final EnviayaRating enviayaRating) {
		try {
			...
			if(ValidatorUtil.isEmpty(enviayaRating.getApiKey())) {
			...
			sourceRates.forEach(item -> {
				val dateStr = DateTimeUtil.formatDate(DateTimeUtil.parseDate(item.getEstimatedDelivery(), Formatter.DATE_DB_FORMAT), Formatter.DATE_SIMPLE_FORMAT);
	...
	protected EnviayaShipment shipmentBooking(final EnviayaShipmentBooking shipmentBooking) {
		try {
			...
			if(ValidatorUtil.isEmpty(shipmentBooking.getApiKey())) {
	...
	protected EnviayaShipment shipmentLookUp(final EnviayaShipmentData data) {
		try {
			...
			if(ValidatorUtil.isEmpty(data.getApiKey())) {
	...
	protected EnviayaTrackingResponse shipmentTracking(final EnviayaTracking tracking) {
		try {
			...
			if(ValidatorUtil.isEmpty(tracking.getApiKey())) {
	...
	protected EnviayaPickupResponse pickupsBooking(final EnviayaPickup enviayaPickup) {
		try {
			...
			if(ValidatorUtil.isEmpty(enviayaPickup.getApiKey())) {
