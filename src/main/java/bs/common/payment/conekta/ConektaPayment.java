package bs.common.payment.conekta;
...
import lombok.Data;
import lombok.val;
import bs.common.util.CalculationUtil;
import bs.common.util.CryptoUtil;
import bs.common.util.PaymentUtil;
...
public abstract class ConektaPayment {
	...
	protected Charge payment(final ConektaData dataBean) throws Exception {
		...
		try {
			...
			if (ValidatorUtil.isNotEmpty(dataBean.getToken()) && ValidatorUtil.isNotEmpty(dataBean.getHolderName())) {
				//##### Customer
				val phone = (ValidatorUtil.isNotEmpty(dataBean.getHolderPhone())) 
						? dataBean.getHolderPhone()
						: PaymentUtil.PAYMENT_DEFAULT_PHONE;
				...
				val item = ConektaItem
							...
							.unit_price(CalculationUtil.penniesInt(dataBean.getTotal()))
				...
				val chargeBean = ConektaCharge
									.builder()
									.id(CryptoUtil.salt())
									.amount(CalculationUtil.penniesInt(dataBean.getTotal()))
									.currency(dataBean.getCurrency())
