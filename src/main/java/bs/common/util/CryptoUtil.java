...
public class CryptoUtil implements Serializable {
	private static final long serialVersionUID = 5409492165368344763L;
	...
	public static String idEncode(final Object id) {
		try {
			...
			if(ValidatorUtil.isNotEmpty(id)) {
				val idStr = ValueUtil.str(id.toString());
				return Base64Util.encode(idStr);
			}
	...
	public static String idDecode(final String idStr) {
		try {
			...
			if(ValidatorUtil.isNotEmpty(idStr)) {
				return Base64Util.decode(idStr);
			}
	...
	public static Long idDecodeLong(final String idStr) {
		try {
			...
			if(ValidatorUtil.isNotEmpty(idStr)) {
				...
				return LongUtil.value(id);
			}
			
		} catch (Exception e) {
			log.error("UCrypto#idDecodeLong error {}", e.getMessage());
		}
	    return null;
	}
	...
	public static Integer idDecodeInteger(final String idStr) {
		try {
			...
			if(ValidatorUtil.isNotEmpty(idStr)) {
				val id = idDecode(idStr);
				return IntegerUtil.value(id);
			}
			
		} catch (Exception e) {
			log.error("UCrypto#idDecodeInteger error {}", e.getMessage());
		}
	    return null;
	}
	...
	public static String textEncode(final String token) {
		try {
			...
			if(ValidatorUtil.isNotEmpty(token)) {
				return CryptoUtil.encryptWithDES(ValueUtil.str(token));
			}
	...
	public static String textDecode(final String token) {
		try {
			...
			if(ValidatorUtil.isNotEmpty(token)) {
			    return CryptoUtil.decryptWithDES(token);
			}
	...
	public static String encrypt(final String data, final String secret) throws Exception {
		...
		val c = Cipher.getInstance(ALGORITHM_AES);
