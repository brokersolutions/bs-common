...
public class CalculationUtil implements Serializable {	
	private static final long serialVersionUID = -6881642884880544900L;
	...
	public static Double percentage(final Double subtotal, final Integer percentage) {
		try {
			...
			var value = DoubleUtil.dValue(IntegerUtil.value(percentage));
	...
	public static BigDecimal percentage(final BigDecimal subtotal, final Integer percentage) {
		try {
			...
			var value = BigDecimalUtil.bValue(percentage);
	...
	public static BigDecimal total(final BigDecimal subTotal, BigDecimal tax) {
		try {
			...
			if(tax == null) {
				tax = BigDecimalUtil.bValue(valueNull);
			}
	...
	public static BigDecimal total(final BigDecimal subTotal, BigDecimal discount, BigDecimal tax) {
		try {
			...
			if(discount == null) {
				discount = BigDecimalUtil.bValue(valueNull);
			}
	...
	public static Integer penniesInt(final Double total) {
		if(total != null) {
			val totalInPennies = IntegerUtil.value(total * 100);
	...
	public static Long penniesLong(final Double total) {
		if(total != null) {
			val totalInPennies = IntegerUtil.value(total * 100);
			return LongUtil.value(totalInPennies);
