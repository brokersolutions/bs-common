...
public class ZipUtil implements Serializable {
	private static final long serialVersionUID = 6396728516438385464L;
    ...
    public static void zipFile(final String sourceFile, final String dirCompressed) {
        if(ValidatorUtil.isNotEmpty(sourceFile) && ValidatorUtil.isNotEmpty(dirCompressed)) {
            ...
            try {
                ...
                if(dirCompressed.contains(ZIP_EXT)) {
                    dirCompressedZip = dirCompressed;
                } else {
                    dirCompressedZip = dirCompressed.concat(ZIP_EXT);
                }
    ...
    public static void zipMultipleFiles(final List<String> srcFiles, final String dirMultiCompressed) {
        if(srcFiles != null && !srcFiles.isEmpty() && ValidatorUtil.isNotEmpty(dirMultiCompressed)) {
            FileOutputStream fos = null;
            ZipOutputStream zipOut = null;
            ...
            try {
                ...
                if(dirMultiCompressed.contains(ZIP_EXT)) {
                    dirMultiCompressedZip = dirMultiCompressed;
                } else {
                    dirMultiCompressedZip = dirMultiCompressed.concat(ZIP_EXT);
                }
        ...
        if(ValidatorUtil.isNotEmpty(sourceFile) && ValidatorUtil.isNotEmpty(dirCompressed)) {
            ...
            try {
                ...
                try {
                    ...
                    log.error("zipDirectory error: Trowing IOException when trying to close zipOut or fos - {}", e.getMessage());
                }
            }
        }
    }
    ...
    public static void zipFilesWithPwd(final List<String> filesToZip, final String zipFilePath, final String password) {
    	val filesToAdd = filesToZip.stream()
					               .map(File::new)
		...
		try (val zipFile = new ZipFile(zipFilePath, password.toCharArray())) {
			
			zipFile.addFiles(filesToAdd, zipParameters);
			
		} catch (ZipException e) {
			log.error("zipFilesWithPwd error: ZipException {} ", e.getMessage());
		} catch (IOException e) {
			log.error("zipFilesWithPwd error: IOException {} ", e.getMessage());
		}
    }
