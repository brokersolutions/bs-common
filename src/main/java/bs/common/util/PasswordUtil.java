...
public class PasswordUtil implements Serializable {
	private static final long serialVersionUID = -8410875580682712373L;
	...
	public static String generateCommonTextPassword() {
		val pwString = generateRandomSpecialCharacters(PWD_LENGTH).concat(generateRandomNumbers(PWD_LENGTH))
				.concat(generateRandomAlphabet(PWD_LENGTH, true)).concat(generateRandomAlphabet(PWD_LENGTH, false))
    ...
    private static String generateRandomSpecialCharacters(int length) {
        val pwdGenerator = new RandomStringGenerator.Builder().withinRange(33, 45).build();
        return pwdGenerator.generate(length);
    }

    private static String generateRandomNumbers(int length) {
        val pwdGenerator = new RandomStringGenerator.Builder().withinRange(48, 57).build();
        return pwdGenerator.generate(length);
    }
    ...
    private static String generateRandomAlphabet(int length, boolean lowerCase) {
        ...
        val pwdGenerator = new RandomStringGenerator.Builder().withinRange(low, hi).build();
        return pwdGenerator.generate(length);
    }
    ...
    public static boolean passwordValidator(final String password) {
    	if(ValidatorUtil.isNotEmpty(password)) {
